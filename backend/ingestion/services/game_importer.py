...
class GameImporter:
    ...
    def run(self, season: str, tournament_slug: str) -> GameImportSummary:
        games = self._client.get_playoff_games(season=season)
        if not games:
            _LOG.warning("No games returned for season=%s", season)
            return GameImportSummary(0, 0, 0)

        with get_connection() as conn:
            with conn.cursor() as cur:
                # Resolve tournament by slug
                cur.execute("SELECT id FROM tournaments WHERE slug = %s", (tournament_slug,))
                t_row = cur.fetchone()
                if not t_row:
                ...
                # Build team external_id → internal UUID map
                cur.execute(
                    "SELECT id, external_id FROM teams WHERE sport = %s",
                    (self._sport,),
                )
                ...
                for g in games:
                    ...
                    if not home_fk or not away_fk:
                        _LOG.debug(
                            "Skipping game %s — teams not found (home=%s away=%s)",
                            g["external_id"],
                    ...
                    upsert_rows.append((
                        tournament_id,
                        None,           # tournament_round_id — linked by import_nba_playoffs
                        g["date"],
                        home_fk,
                        away_fk,
                        g["home_score"],
                ...
                query = """
                    INSERT INTO games
                        (id, tournament_id, tournament_round_id, date,
                         home_team_id, away_team_id, home_score, away_score,
                         winner_team_id, status, external_id, created_at, updated_at)
                    VALUES %s
                ...
                results = execute_values(cur, query, upsert_rows, template=template, fetch=True)
                if results is None:
                    return GameImportSummary(0, 0, len(games))
                ...
                _LOG.info(
                    "Game import done: %d pushed (%d new, %d updated)",
                    len(upsert_rows), inserted_count, updated_count,
                )
